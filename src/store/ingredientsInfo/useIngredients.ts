...
export function useIngredients(): [IIngredientLocal[], ReturnType<typeof selectLoadIngredientsInfo>] {
  const dispatch = useAppDispatch();
  const ingredients = useSelector(selectIngredientsInfo);
  const { isLoading, error } = useSelector(selectLoadIngredientsInfo);

  useEffect(() => {
    if (ingredients.length === 0) dispatch(loadIngredientsInfo({ url: LIST_URL, params: { i: 'list' } }));
  }, []);
